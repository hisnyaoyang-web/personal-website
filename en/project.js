...
const gallery=project.gallery?.length?`<section class="detail-gallery"><p class="kicker">PROJECT EVIDENCE</p><div class="detail-gallery-grid">${project.gallery.map(item=>`<figure><div class="detail-gallery-image"><img src="${item.image}" alt="${item.caption}" loading="lazy" draggable="false"></div><figcaption>${item.caption}</figcaption></figure>`).join('')}</div></section>`:'';
const media=project.video?`<figure class="detail-media video-media"><video controls playsinline preload="metadata" controlslist="nodownload noremoteplayback" disablepictureinpicture disableremoteplayback draggable="false" oncontextmenu="return false"${project.videoPoster||project.image?` poster="${project.videoPoster||project.image}"`:''}><source src="${project.video}">Your browser does not support video playback.</video><figcaption>Project demo video</figcaption></figure>`:project.image?`<figure class="detail-media"><img src="${project.image}" alt="${project.title} project visual" draggable="false"><figcaption>Current project asset · fuller process evidence to come</figcaption></figure>`:`<div class="detail-media missing"><p>MEDIA<br>TO BE ADDED</p><span>Screenshots, flow diagrams, or a demo video for this project are coming</span></div>`;
