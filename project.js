...
const gallery=project.gallery?.length?`<section class="detail-gallery"><p class="kicker">PROJECT EVIDENCE · 项目佐证</p><div class="detail-gallery-grid">${project.gallery.map(item=>`<figure><div class="detail-gallery-image"><img src="${item.image}" alt="${item.caption}" loading="lazy" draggable="false"></div><figcaption>${item.caption}</figcaption></figure>`).join('')}</div></section>`:'';
const media=project.video?`<figure class="detail-media video-media"><video controls playsinline preload="metadata" controlslist="nodownload noremoteplayback" disablepictureinpicture disableremoteplayback draggable="false" oncontextmenu="return false"${project.videoPoster||project.image?` poster="${project.videoPoster||project.image}"`:''}><source src="${project.video}">你的浏览器暂不支持播放该视频。</video><figcaption>项目演示视频</figcaption></figure>`:project.image?`<figure class="detail-media"><img src="${project.image}" alt="${project.title}项目视觉" draggable="false"><figcaption>现有项目素材 · 后续将补充更完整的过程证据</figcaption></figure>`:`<div class="detail-media missing"><p>MEDIA<br>TO BE ADDED</p><span>该项目的界面截图、流程图或演示视频尚待补充</span></div>`;
